...
namespace MFA_TOTP
{
    public class Tools
    {
        ...
        public void Write(String path, String pin, String key)
        {
            // Initilize class for XML
            ...
            tools._key = EncryptStringAES(key, pin);
        ...
        {
            // Read Pin Hash From XML File
            System.Xml.Serialization.XmlSerializer reader = new System.Xml.Serialization.XmlSerializer(typeof(Tools));
            StreamReader fileRead = new StreamReader(path);
            Tools test = (Tools)reader.Deserialize(fileRead);
            fileRead.Close();

            bool check = Verify(pin, test._pin);
            return check;
        }

        public String GetKey(String path, String pin)
        {
            // Read Pin Hash From XML File
            System.Xml.Serialization.XmlSerializer reader = new System.Xml.Serialization.XmlSerializer(typeof(Tools));
            StreamReader fileRead = new StreamReader(path);
            ...
            String secret = DecryptStringAES(test._key, pin);
            return secret;
        }
